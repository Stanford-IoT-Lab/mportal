...
module.exports = new lang.Class({
        ...
        return Q.all([medical_record.getWeightRecords(db, patientId),
            ...
            return Q.all(records.map(function(r) {
                ...
                return pictObj.getSharedLink().then(function(url) {
                    ...
                    return ({ record_type: r.record_type, picture_url: url, collect_time: r.collect_time });
                });
            }));
    ...
    insertOther: function(obj, type, time) {
        var now = new Date;
        if (time === undefined)
            time = now;
        return db.transaction(function(db) {
            return medical_record.insertOther(db, this._row.id, type, now, time, obj.serialize()).then(function() {
                if (this.sync_enabled) {
                    return this._doctorPortal.getPatient(this._user.account).then(function(syncId) {
                        return obj.getSharedLink().then(function(url) {
                            return this._doctorPortal.insertMedicalRecord(syncId, { record_type: type, picture_url: url, collect_time: time });
                        }.bind(this));
                    }.bind(this));
                }
            }.bind(this));
