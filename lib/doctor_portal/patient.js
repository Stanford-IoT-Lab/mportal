...
module.exports = {
    ...
    _mapRecord: function(patientId, record) {
        if (record.record_type === 'height')
            ...
            var descr = record.description;
            if (!descr) {
                if (record.record_type === 'catscan')
                    descr = "CT scan";
                else if (record.record_type === 'xray')
                    descr = "X-ray";
            }
            return [{ patient_id: patientId, description: descr, type: 'other', picture: record.picture_url, timestamp: record.collect_time }];
        } else
            return [];
    },
