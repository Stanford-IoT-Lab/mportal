...
const Dialog = new lang.Class({
    ...
    handleGeneric: function(analyzer) {
        ...
        if (analyzer.isSpecial) {
            ...
            case 'tt:root.special.help':
                ...
                this.reply("Here are some examples of sentences I understand:");
                this.reply("\"My height is 183 cm\"");
                this.reply("\"Insert cat scan\"");
                this.reply("\"Share with dr John Smith\"");
                this.reply("\"What is my blood pressure?\"");
                this.reply("\"What was my weight on April 2nd?\"");
                this.reply("\"What is my diagnosis?\"");
                if (this.expecting !== null) {
    ...
    fail: function(verbose) {
        this.reply("Sorry, I did not understand that. Can you rephrase it?");
        if (verbose)
            this.reply("(Say \"help\" for a list of common phrases I understand)");
        return true;
    },
...
const AskDialog = new lang.Class({
    ...
    _getLastData: function(analyzer) {
    ...
    _getOldData: function(analyzer) {
        if (analyzer.type === 'weight')
            return this.manager.patient.getOldWeight(analyzer.when);
        else if (analyzer.type === 'height')
            return this.manager.patient.getOldHeight(analyzer.when);
        else if (analyzer.type === 'gender')
            return this.manager.patient.getOldGender(analyzer.when);
        else if (analyzer.type === 'diagnosis')
            return this.manager.patient.getOldDiagnosis(analyzer.when);
        else if (analyzer.type === 'body_temperature')
            return this.manager.patient.getOldBodyTemperature(analyzer.when);
        else if (analyzer.type === 'heart_rate')
            return this.manager.patient.getOldHeartRate(analyzer.when);
        else if (analyzer.type === 'blood_pressure')
            return this.manager.patient.getOldBloodPressure(analyzer.when);
        else if (analyzer.type === 'labresults')
            return this.manager.patient.getOldLabResults(analyzer.when);
        else
            throw new TypeError();
    },

    _getData: function(analyzer) {
        if (analyzer.when === null)
            return this._getLastData(analyzer);
        else
            return this._getOldData(analyzer);
    },
    ...
    handle: function(analyzer) {
        ...
        this._getData(analyzer).then(function(info) {
            ...
            if (analyzer.type === 'weight')
                this.reply(info.weight_kgs.toFixed(1) + ' kgs');
            else if (analyzer.type === 'height')
                this.reply(info.height_cms.toFixed(0) + ' cms');
            else if (analyzer.type === 'gender')
                this.reply(info.gender);
            else if (analyzer.type === 'diagnosis')
                this.reply(info.cancer_type + ' cancer, stage ' + (info.substage !== null ? (info.stage + ' ' + info.substage) : info.stage));
            else if (analyzer.type === 'body_temperature')
                this.reply(info.temp_C.toFixed(1) + ' ℃');
            else if (analyzer.type === 'heart_rate')
                this.reply(info.heart_rate_bpm.toFixed(0) + ' bpm');
            else if (analyzer.type === 'blood_pressure')
                this.reply(info.blood_pressure_mmHg.toFixed(0) + ' mmHg');
            else if (analyzer.type === 'labresults') {
                this.reply('White blood cell count: ' + info.white_blood_cell_count.toFixed(0) + ' K/uL');
                this.reply('Hematocrit: ' + info.hematocrit_percent.toFixed(0) + '%');
                this.reply('Hemoglobin: ' + info.hemoglobin.toFixed(0) + ' g/dL');
                this.reply('Platelet count: ' + info.platelet_count.toFixed(0) + ' K/uL');
            }
            this.switchToDefault();
        }.bind(this)).done();
        return true;
...
const AskWhenDialog = new lang.Class({
    ...
    handle: function(analyzer) {
        ...
        this._getData(analyzer).then(function(info) {
            this.reply(info.collect_time.toLocaleString());
            this.switchToDefault();
        }.bind(this)).done();
        return true;
    }
...
const InsertGenericDialog = new lang.Class({
    ...
    _continue: function() {
        ...
        } else if (this.kind === 'other' && this.description === null) {
            return this.ask(ValueCategory.RawString, "What kind of record is it?");
        } else if (this.time === null) {
            return this.ask(ValueCategory.Date, "When did you obtain it?");
        } else {
            this.manager.patient.insertOther(this.obj, this.kind, this.description, this.time).then(function() {
    ...
    _complete: function() {
        if (this.kind === 'weight') {
            return this.manager.patient.insertWeight(this.value, this.time).then(function() {
                this.reply("Ok, I added a new weight record with value " + this.value.toFixed(1) + " kgs.");
            }.bind(this));
        } else if (this.kind === 'height') {
            return this.manager.patient.insertHeight(this.value * 100, this.time).then(function() {
                this.reply("Ok, I added a new height record with value " + (this.value * 100).toFixed(0) + " cms.");
            }.bind(this));
        } else if (this.kind === 'diagnosis') {
            ...
            return this.manager.patient.insertBodyTemperature(this.value, this.time).then(function() {
                this.reply("Ok, I added a new body temperature record with value " + this.value.toFixed(1) + " C.");
            }.bind(this));
        } else if (this.kind === 'heart_rate') {
            return this.manager.patient.insertHeartRate(this.value, this.time).then(function() {
                this.reply("Ok, I added a new heart rate record with value " + this.value.toFixed(0) + " bpm.");
            }.bind(this));
        } else if (this.kind === 'blood_pressure') {
            return this.manager.patient.insertBloodPressure(this.value, this.time).then(function() {
                this.reply("Ok, I added a new blood pressure record with value " + this.value.toFixed(0) + " mmHg.");
            }.bind(this));
        } else if (this.kind === 'labresults') {
...
const InitializationDialog = new lang.Class({
    ...
    _checkName: function() {
        ...
        if (this.manager.user.name === '')
            return this.ask(ValueCategory.RawString, "What is your full name?");
        else
            return this.ask(ValueCategory.YesNo, "Is your full name " + this.manager.user.name + "?");
    },
    ...
    _handleNameResponse: function(word) {
        if (word.isYes) {
            this.full_name = this.manager.user.name;
            this.manager.patient.store({ full_name: this.full_name,
                                         nick_name: this.full_name })
                .then(function() {
                    this.reply("Hi " + this.full_name + ", nice to meet you.");
                    this._continue();
                }.bind(this));
            this.expecting = null;
            return true;
        } else {
            return this.ask(ValueCategory.RawString, "Ok, what's your full name then?");
        }
    },
    ...
    start: function() {
        ...
        setTimeout(function() {
            this.reply("Hello! My name is Leland EMRLiteBot, and I'm your medical assistant.");
    ...
    handleRaw: function(command) {
        if (this.expecting === ValueCategory.RawString) {
            if (this.full_name === null) {
                this.full_name = command;
                this.manager.patient.store({ full_name: this.full_name,
                                             nick_name: this.manager.user.name })
                    .then(function() {
                        this.reply("Hi " + this.full_name + ", nice to meet you.");
                        this._continue();
                    }.bind(this));
